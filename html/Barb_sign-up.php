...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen($passwords) < 8 || !preg_match("/[a-z]/i", $passwords) || !preg_match("/[0-9]/", $passwords)) {
      ...
      $dbconn = require "db.php";
        ...
        $sql = "INSERT INTO barber (Barb_Firstname, Barb_Lastname, Barb_ContactNo, Barb_Username, Barb_Password)
            VALUES (?, ?, ?, ?, ?)";
        ...
        try {
            if ($stmt->execute()) {
                header("Location: Barb_login.php");
                exit;
            }
        } catch (Exception $e) {
            ...
            <form method="post">
                <div class="signup">
                    ...
                    <div class="signup_link">
                        Already have an account? <a href="Barb_login.php">Login</a>
                    </div>
            </form>
        </div>
    </div>
